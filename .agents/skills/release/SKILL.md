---
name: release
description: Prepare, validate, and publish a NextBitDeskify release. Not for version bumps without release intent.
version: 1.2.0
allowed-tools:
  - Bash
  - Read
...
Use this skill when preparing or executing a NextBitDeskify release.
...
- `src-tauri/Cargo.lock` → `version` for package `nextbitdeskify`
...
2. [ ] Confirm the version is not already on npm: `npm view nextbitdeskify-cli@X.Y.Z version` should return 404 before publishing
...
7. [ ] Verify npm published the exact package: `npm view nextbitdeskify-cli@X.Y.Z version gitHead dist.tarball --json`
8. [ ] Verify latest now resolves to the release: `npm view nextbitdeskify-cli version`
9. [ ] Record Quality & Testing status separately: `gh run list --workflow=quality-and-test.yml --limit 3`

npm publishes through Trusted Publishing from `.github/workflows/npm-publish.yml`. Configure npm package settings with GitHub Actions, `AquilaWilfred/NextBitDeskify`, workflow file `npm-publish.yml`, and no environment. Local `npm publish` is only a fallback if CI or registry state blocks the trusted path.

Keep release surfaces separate in the final status:

- npm registry: the authority for `nextbitdeskify-cli` installability and CLI/npm issue closeout.
...
```markdown
<div align="center">
<img src="https://gw.alipayobjects.com/zos/k/fa/logo-modified.png" alt="NextBitDeskify Logo" width="120" height="120" style="border-radius:50%" />
<h1 style="margin: 12px 0 6px;">NextBitDeskify VX.Y.Z</h1>
...
- Do not reply to GitHub issues or close them as released until `npm view nextbitdeskify-cli@X.Y.Z version` returns the expected version. `npm view nextbitdeskify-cli version` alone is not enough because `latest` can point at a different commit than the fix under review.
