---
name: use-nextbitdeskify
description: "Package any website into a lightweight desktop app using NextBitDeskify (Tauri/Rust). Use when the user wants to: wrap a URL as a native app, build a desktop app from a website, use NextBitDeskify CLI to package a page, set up proxy for a packaged app, customize app icons or bundle IDs, or mentions 'nextbitdeskify', 'tauri package', 'website to app', 'wrap site'. Also trigger when the user asks about NextBitDeskify CLI options, proxy configuration for packaged apps, or icon handling."
version: 1.0.0
allowed-tools:
  - Bash
  - Read
...
# NextBitDeskify - Website to Desktop App

NextBitDeskify wraps any webpage into a native desktop app via Tauri (Rust + system WebView). Output is ~5MB (vs Electron's ~150MB). Run the commands below from the root of this NextBitDeskify repository.
...
NextBitDeskify auto-fetches icons from multiple services (logo.dev, brandfetch, clearbit, Google favicons, direct favicon.ico). However:
...
- NextBitDeskify auto-converts to platform format: `.icns` (macOS), `.ico` (Windows), `.png` (Linux)
- macOS icons get a squircle mask automatically
- If an old icon exists in `src-tauri/icons/<name>.icns`, NextBitDeskify reuses it — rename/remove it to force re-fetch
...
node dist/cli.js "<URL>" \
  --name <AppName> \
  --identifier <com.nextbitdeskify.xxx> \
...
Not supported. NextBitDeskify uses system WebView (WKWebView on macOS, WebView2 on Windows, WebKitGTK on Linux), not a full Chrome browser. Use `--inject` to add custom JS/CSS as an alternative.

### Linux Wayland Input Issues

If an AppImage opens but buttons cannot be clicked or keyboard input does not reach the page on a pure Wayland compositor, especially niri, first rebuild with the latest `nextbitdeskify-cli`. Then try the native WebKit path:

```bash
NEXTBITDESKIFY_LINUX_WEBKIT_SAFE_MODE=0 ./YourApp.AppImage
```

If that produces a blank window on the same system, re-enable the conservative WebKit workaround:

```bash
NEXTBITDESKIFY_LINUX_WEBKIT_SAFE_MODE=1 ./YourApp.AppImage
```
...
node dist/cli.js "https://example.com" \
  --name MyApp \
  --identifier com.nextbitdeskify.myapp \
