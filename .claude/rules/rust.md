# NextBitDeskify Rust + Tauri Rules

> NextBitDeskify-specific Rust + Tauri rules. Standard Rust hygiene is assumed: `?` over `unwrap()`, `cargo clippy` clean, `cargo fmt` before commit.

## NextBitDeskify-Specific
...
- No `tauriConf: any` or other untyped config bags. Use `NextBitDeskifyTauriConfig`.
