...
export default abstract class BaseBuilder {
  protected options: NextBitDeskifyAppOptions;

  protected constructor(options: NextBitDeskifyAppOptions) {
  ...
  async start(url: string) {
    logger.info('NextBitDeskify dev server starting...');
  ...
  async buildAndCopy(url: string, target: string, logSuccess = true) {
    const { name = 'nextbitdeskify-app' } = this.options;
    await mergeConfig(url, this.options, tauriConfig);
    ...
    if (isLinuxAppImage && !buildEnv.NO_STRIP && this.options.debug) {
      logger.warn(
        '⚠ AppImage strip step can fail on glibc 2.38+; NextBitDeskify will auto-retry with NO_STRIP=1.',
      );
    }
  ...
  protected getBinaryName(appName: string): string {
    ...
    return `nextbitdeskify-${nameToUse}${extension}`;
  }
