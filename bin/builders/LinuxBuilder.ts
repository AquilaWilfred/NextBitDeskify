...
export default class LinuxBuilder extends BaseBuilder {
  ...
  constructor(options: NextBitDeskifyAppOptions) {
    super(options);
  ...
  getFileName() {
    const { name = 'nextbitdeskify-app', targets } = this.options;
  ...
  private async createArchPackageFromDeb({
    ...
    const workDir = path.resolve('.nextbitdeskify-arch-package');
    ...
    try {
      ...
      // the payload already ships NextBitDeskify's own com.nextbitdeskify.<name>.desktop.
