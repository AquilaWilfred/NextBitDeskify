...
export async function detectPackageManager(): Promise<'pnpm' | 'npm'> {
  ...
  if (
    ...
    if (!(await detectNpm(execa))) {
      throw new Error(
        `Detected pnpm ${normalizedPnpmVersion}, but NextBitDeskify is pinned to ${packageJson.packageManager}. Install npm so NextBitDeskify can fall back, or use pnpm ${pinnedPnpmMajor}.x to match the project pin.`,
      );
    }

    logger.warn(
      `✼ Detected pnpm ${normalizedPnpmVersion}, but NextBitDeskify is pinned to ${packageJson.packageManager}; using npm for package management instead.`,
    );
    packageManagerCache = 'npm';
    return 'npm';
  }
