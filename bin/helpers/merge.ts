 ...
 * merged into nextbitdeskify.json. Exposed for snapshot testing so option drift
...
export function buildWindowConfigOverrides(
  options: NextBitDeskifyAppOptions,
...
function asSupportedPlatform(platform: NodeJS.Platform): SupportedPlatform {
  if (platform !== 'win32' && platform !== 'darwin' && platform !== 'linux') {
    throw new Error(
      `NextBitDeskify only supports win32, darwin, and linux; detected '${platform}'.`,
    );
  }
  return platform;
}
...
async function handleLocalFile(
  ...
  tauriConf: NextBitDeskifyTauriConfig,
): Promise<void> {
  ...
  if (pathExists) {
    ...
    tauriConf.nextbitdeskify.windows[0].url = fileName;
    tauriConf.nextbitdeskify.windows[0].url_type = 'local';
  } else {
    tauriConf.nextbitdeskify.windows[0].url_type = 'web';
  }
}
...
async function mergeLinuxConfig(
  ...
  const desktopFileName = `com.nextbitdeskify.${linuxName}.desktop`;
...
async function mergeIcons(
  ...
  tauriConf.nextbitdeskify.system_tray_path = trayIconPath;
...
async function injectCustomCode(
  ...
  if (inject?.length > 0) {
    ...
    tauriConf.nextbitdeskify.inject = files;
    await combineFiles(files, injectFilePath);
  } else {
    tauriConf.nextbitdeskify.inject = [];
    await fsExtra.writeFile(injectFilePath, '');
  }

  tauriConf.nextbitdeskify.proxy_url = proxyUrl || '';
  tauriConf.nextbitdeskify.multi_instance = multiInstance;
  tauriConf.nextbitdeskify.multi_window = multiWindow;
...
export async function mergeConfig(
  ...
  const linuxBinaryName = `nextbitdeskify-${generateLinuxPackageName(name)}`;
  tauriConf.mainBinaryName =
    platform === 'linux'
      ? linuxBinaryName
      : `nextbitdeskify-${generateIdentifierSafeName(name)}`;
  ...
  if (userAgent.length > 0) {
    tauriConf.nextbitdeskify.user_agent[currentPlatform] = userAgent;
  }
  tauriConf.nextbitdeskify.system_tray[currentPlatform] = showSystemTray;
