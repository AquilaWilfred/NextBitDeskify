...
import { NextBitDeskifyError } from '@/utils/error';
import { NextBitDeskifyAppOptions, NextBitDeskifyCliOptions } from '@/types';

function resolveAppName(name: string, platform: NodeJS.Platform): string {
  const domain = getDomain(name) || 'nextbitdeskify';
...
export function resolveLocalAppName(
  ...
  const baseName = path.parse(filePath).name || 'nextbitdeskify-app';
  if (platform === 'linux') {
    return generateLinuxPackageName(baseName) || 'nextbitdeskify-app';
  }
  const normalized = baseName
    ...
    .replace(/\s+/g, ' ')
    .trim();
  return normalized || 'nextbitdeskify-app';
}
...
export default async function handleOptions(
  options: NextBitDeskifyCliOptions,
  url: string,
): Promise<NextBitDeskifyAppOptions> {
  ...
  if (name && !isValidName(name, platform)) {
    ...
    if (isActions) {
      ...
      throw new NextBitDeskifyError(errorMsg);
    }
  }

  const resolvedName = name || 'nextbitdeskify-app';

  const appOptions: NextBitDeskifyAppOptions = {
    ...options,
    name: resolvedName,
