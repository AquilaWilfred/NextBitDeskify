...
export interface NextBitDeskifyCliOptions {
  // Application name
  name?: string;
  ...
  // Recursive copy, when url is a local file path, if this option is enabled, the url path file and all its subFiles will be copied to the nextbitdeskify static file folder, default off
  useLocalFile: false;
  ...
  // Force navigation to stay inside the NextBitDeskify window even for external links
...
export interface NextBitDeskifyAppOptions extends NextBitDeskifyCliOptions {
  identifier: string;
}
...
export interface NextBitDeskifyConfig {
...
export interface NextBitDeskifyTauriConfig {
  productName?: string;
  identifier?: string;
  version?: string;
  mainBinaryName?: string;
  nextbitdeskify: NextBitDeskifyConfig;
  bundle: {
    icon?: string[];
    resources?: string[];
    targets?: string[];
