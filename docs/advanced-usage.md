...
NextBitDeskify automatically provides user-friendly download error notifications:
...
Send messages between web content and NextBitDeskify container.
...
Configure window properties in `nextbitdeskify.json`:
...
`hide_title_bar` is the `nextbitdeskify.json` key (the CLI exposes it as `--hide-title-bar`). It is only supported on macOS and is ignored on Windows and Linux.
...
```bash
nextbitdeskify ./my-app/index.html --name my-static-app --use-local-file
```

Requirements: NextBitDeskify CLI >= 3.0.0

## macOS Media Permissions

By default, apps built with NextBitDeskify do not request camera or microphone access. For sites that require these (for example, video conferencing or voice input), pass the relevant flags at build time:

```bash
nextbitdeskify https://chatgpt.com --name ChatGPT --microphone
nextbitdeskify https://meet.google.com --name GoogleMeet --camera --microphone
```
...
```bash
nextbitdeskify https://gmail.com --name "Gmail Work"
nextbitdeskify https://gmail.com --name "Gmail Personal"
```

NextBitDeskify now generates a different app identifier for each `URL + name` pair, so these apps can be installed as separate desktop apps instead of resolving to the same app.

For advanced cases, NextBitDeskify also supports a hidden `--identifier` option if you need to pin the bundle identifier explicitly:

```bash
nextbitdeskify https://gmail.com --name "Gmail Work" --identifier com.example.gmail.work
```
...
This script reads the configuration and packages the specified app in watch mode, with hot updates for `nextbitdeskify-cli` code changes.
...
- `--nextbitdeskify-cli`: add GitHub Actions related checks
