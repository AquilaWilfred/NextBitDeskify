...
npx nextbitdeskify-cli [url] [options]
...
**Optional parameter**: If not provided, NextBitDeskify will automatically fetch the website's icon and convert to the appropriate format. For custom icons, visit [icon-icons](https://icon-icons.com) or [macOSicons](https://macosicons.com/#/).
...
Sets whether to disable web shortcuts in the original NextBitDeskify container, defaults to `false`.
...
Enable NextBitDeskify's in-page Find UI. Default is `false`. When enabled, users can press `Cmd/Ctrl+F` to open Find, `Cmd/Ctrl+G` to jump to the next match, and `Cmd/Ctrl+Shift+G` to jump to the previous match.
...
Keeps every clicked link (even pointing to other domains) inside the NextBitDeskify window instead of letting the OS open an external browser or helper. Default is `false`.
...
A simpler way to keep trusted domains and their subdomains inside the app. This is useful for workspace callbacks and enterprise SSO flows, for example Slack plus Okta. NextBitDeskify compiles this list into `internal_url_regex`; if `--internal-url-regex` is also set, the explicit regex wins.
...
- ARM64 support enables NextBitDeskify apps to run on ARM-based Linux devices, including Linux phones (postmarketOS, Ubuntu Touch), Raspberry Pi, and other ARM64 Linux systems.
- Use `--target appimage-arm64` for portable ARM64 applications that work across different ARM64 Linux distributions.
- Use `--targets zst` on Arch Linux based distributions to produce a `.pkg.tar.zst` package directly. NextBitDeskify follows Tauri's AUR packaging guidance by building the Linux package payload first, then emitting Arch package metadata and zstd-compressed output. Requires `binutils` (for `ar`) and `libarchive` (for `bsdtar`).
...
```shell
nextbitdeskify https://github.com --name GitHub --no-bundle
```
...
nextbitdeskify https://github.com --name GitHub --show-system-tray --start-to-tray
```
...
nextbitdeskify https://flutter.dev --name FlutterApp --wasm
```
...
nextbitdeskify https://planka.example.com --name PlankApp --enable-drag-drop
```
...
nextbitdeskify https://github.com --name GitHub --keep-binary
```
...
This option is macOS-only and is intended for local development or quick testing. When enabled, NextBitDeskify builds an `.app` bundle, copies it into `/Applications`, replaces any existing app with the same name, and removes the local bundle after a successful install. If the install fails, the local `.app` is kept in the current working directory.

```shell
--install

# Example: Build and install directly to /Applications
nextbitdeskify https://github.com --name GitHub --install
```
...
nextbitdeskify https://meet.google.com --name Meet --camera
```
...
nextbitdeskify https://meet.google.com --name Meet --camera --microphone
```
...
nextbitdeskify https://chat.example.com --name ChatApp --multi-instance
```
...
nextbitdeskify https://chat.example.com --name ChatApp --multi-window
```
...
Enable recursive copying. When the URL is a local file path, enabling this option will copy the folder containing the file specified in the URL, as well as all sub-files, to the NextBitDeskify static folder. This is disabled by default.

```shell
--use-local-file

# Basic static file packaging
nextbitdeskify ./my-app/index.html --name "my-app" --use-local-file
```
...
```shell
# Run the NextBitDeskify CLI via Docker (AppImage builds need FUSE access)
