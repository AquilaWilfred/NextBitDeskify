...
**Problem:**
When building NextBitDeskify or using the CLI, you encounter an error like:
...
NextBitDeskify's dependencies require Rust edition2024 support, which is only available in Rust 1.85.0 or later. Specifically:
...
`.deb` package cannot be installed by the system package manager, and older NextBitDeskify
versions always built a `.deb` first.

**Solution:**

NextBitDeskify now picks the default bundle target from `/etc/os-release`: RPM-based
...
```bash
nextbitdeskify https://github.com --name GitHub --targets rpm        # RPM package
nextbitdeskify https://github.com --name GitHub --targets appimage   # portable AppImage
```
...
NextBitDeskify CLI now automatically retries AppImage builds with `NO_STRIP=1` when linuxdeploy fails to strip the binary. To skip the strip step from the very first attempt (or when scripting your own builds), set the variable manually:

```bash
NO_STRIP=1 nextbitdeskify https://example.com --name MyApp --targets appimage
```
...
```bash
nextbitdeskify https://example.com --name MyApp --targets deb
```
...
Building inside NextBitDeskify's Docker image matches the library layout the AppImage bundler expects:
...
**Problem:**
You installed Rust but NextBitDeskify still reports "cargo: command not found".

**Solution:**

NextBitDeskify CLI automatically reloads the Rust environment, but if issues persist:
...
NextBitDeskify CLI uses the official npm and Rust sources by default. If downloads are slow in China, opt in to CN mirrors:
...
```bash
# Navigate to nextbitdeskify-cli installation directory
cd %LOCALAPPDATA%\pnpm\global\5\.pnpm\nextbitdeskify-cli@VERSION\node_modules\nextbitdeskify-cli
...
```bash
nextbitdeskify https://example.com --width 1200 --height 800
```
...
```bash
# macOS
nextbitdeskify https://example.com --icon ./icon.icns

# Windows
nextbitdeskify https://example.com --icon ./icon.ico

# Linux
nextbitdeskify https://example.com --icon ./icon.png
```

NextBitDeskify can automatically convert icons, but providing the correct format is more reliable.
...
**Problem:**
Some website features don't work in the NextBitDeskify app.
...
1. **Set custom User Agent:**
   ...
   ```bash
   nextbitdeskify https://example.com --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
   ```

2. **Inject custom JavaScript:**

   ```bash
   nextbitdeskify https://example.com --inject ./fix.js
   ```

   For pages that need periodic reloads, you can keep this behavior in a small injected script instead of adding a dedicated NextBitDeskify option:
   ...
   ```bash
   nextbitdeskify https://news.ycombinator.com --name HackerNews --inject ./refresh.js
   ```
...
The ~5MB number is the installer/app size on disk, not runtime memory. At runtime NextBitDeskify renders through your system WebView (WebKitWebProcess on Linux, WKWebView on macOS), and that process's memory is governed by the engine and the page you load, not by NextBitDeskify. A heavy SPA like Gemini, Slack, or ChatGPT uses a comparable amount opening in any WebKitGTK browser such as GNOME Web. NextBitDeskify adds very little on top of the WebView, so there is no NextBitDeskify-side setting that meaningfully lowers it. This is inherent to using the system WebView and is the trade-off for the small binary size.
...
**Problem:**
`npm install -g nextbitdeskify-cli` fails with permission errors.
...
npx nextbitdeskify-cli https://example.com

# Option 2: Fix npm permissions
npm config set prefix ~/.npm-global
echo 'export PATH=~/.npm-global/bin:$PATH' >> ~/.bashrc
source ~/.bashrc
npm install -g nextbitdeskify-cli
