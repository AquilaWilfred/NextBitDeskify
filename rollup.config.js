...
const devPlugins = !isProduction ? [nextbitdeskifyCliDevPlugin()] : [];
...
function nextbitdeskifyCliDevPlugin() {
  let devChildProcess;
  let cliChildProcess;
  ...
  return {
    name: "nextbitdeskify-cli-dev-plugin",
    buildEnd() {
      const command = "node";
      ...
      cliChildProcess.on("close", async (code) => {
        ...
        const command = `${packageManager} run tauri dev --config ./src-tauri/.nextbitdeskify/tauri.conf.json -- --features cli-build`;
