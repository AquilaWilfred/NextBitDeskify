...
pub fn set_app_menu(
    ...
    let menu = Menu::with_items(
        ...
        &[
            ...
            &navigation_menu(app)?,
            &window_submenu,
            &help_menu(app, &nextbitdeskify_menu_item_title)?,
        ],
    )?;
...
fn app_menu(app: &AppHandle<Wry>) -> tauri::Result<Submenu<Wry>> {
    let app_menu = Submenu::new(app, "NextBitDeskify", true)?;
...
pub fn handle_menu_click(app_handle: &AppHandle, id: &str) {
    match id {
        ...
        "go_home" => {
            if let Some(window) = app_handle.get_webview_window("nextbitdeskify") {
                let _ = window.eval("window.location.href = window.nextbitdeskifyConfig.url");
            }
        }
        "copy_url" => {
            if let Some(window) = app_handle.get_webview_window("nextbitdeskify") {
        ...
        "find" => {
            if let Some(window) = app_handle.get_webview_window("nextbitdeskify") {
                let _ = window.eval("window.nextbitdeskifyFind?.open()");
            }
        }
        "find_next" => {
            if let Some(window) = app_handle.get_webview_window("nextbitdeskify") {
                let _ = window.eval("window.nextbitdeskifyFind?.next()");
            }
        }
        "find_previous" => {
            if let Some(window) = app_handle.get_webview_window("nextbitdeskify") {
                let _ = window.eval("window.nextbitdeskifyFind?.previous()");
            }
        }
        "clear_cache_restart" => {
            if let Some(window) = app_handle.get_webview_window("nextbitdeskify") {
