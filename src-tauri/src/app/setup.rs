...
pub fn set_system_tray(
    ...
    if !show_system_tray {
        app.remove_tray_by_id("nextbitdeskify-tray");
        return Ok(());
    }
    ...
    app.app_handle().remove_tray_by_id("nextbitdeskify-tray");
    ...
    let mut tray_builder = TrayIconBuilder::new()
        ...
        .on_menu_event(move |app, event| match event.id().as_ref() {
            ...
            "hide_app" => {
                if let Some(window) = app.get_webview_window("nextbitdeskify") {
        ...
        .on_tray_icon_event(move |tray, event| {
            if let TrayIconEvent::Click { button, .. } = event {
                if button == tauri::tray::MouseButton::Left {
                    if let Some(window) = tray.app_handle().get_webview_window("nextbitdeskify") {
    ...
    if let Some(icon) = resolved_icon {
        ...
        eprintln!("[NextBitDeskify] No tray icon available; tray will build without an icon.");
    }
...
pub fn set_global_shortcut(
    ...
    let shortcut_hotkey = match Shortcut::from_str(&shortcut) {
        Ok(s) => s,
        Err(error) => {
            eprintln!("[NextBitDeskify] Invalid activation shortcut '{shortcut}': {error}");
            return Ok(());
        }
    };
    ...
    if let Err(error) = app_handle.plugin(
        ...
        eprintln!(
            "[NextBitDeskify] Failed to register global shortcut plugin '{shortcut}': {error}; continuing without it."
        );
        return Ok(());
    }

    if let Err(error) = app.global_shortcut().register(shortcut_hotkey) {
        eprintln!("[NextBitDeskify] Failed to bind global shortcut '{shortcut}': {error}");
    }
