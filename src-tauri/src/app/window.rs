use crate::app::config::NextBitDeskifyConfig;
use crate::util::{
...
pub struct MultiWindowState {
    pub nextbitdeskify_config: NextBitDeskifyConfig,
    pub tauri_config: Config,
    next_window_index: AtomicU32,
}

impl MultiWindowState {
    pub fn new(nextbitdeskify_config: NextBitDeskifyConfig, tauri_config: Config) -> Self {
        Self {
            nextbitdeskify_config,
            tauri_config,
    ...
    fn next_window_label(&self) -> String {
        ...
        format!("nextbitdeskify-{index}")
    }
}

pub fn set_window(
    app: &AppHandle,
    config: &NextBitDeskifyConfig,
    tauri_config: &Config,
) -> tauri::Result<WebviewWindow> {
    build_window_with_label(app, config, tauri_config, "nextbitdeskify")
}
...
fn build_window_with_label(
    ...
    let window_config = config.windows.first().ok_or_else(|| {
        tauri::Error::Io(std::io::Error::new(
            ...
            "nextbitdeskify.json must define at least one window configuration",
        ))
    })?;
        ...
        "web" => {
            let parsed = window_config.url.parse().map_err(|err| {
                tauri::Error::Io(std::io::Error::new(
                    ...
                    format!(
                        "Invalid 'web' url '{}' in nextbitdeskify.json: {err}",
                        window_config.url
                    ),
                ))
            })?;
...
fn build_window(
    ...
    let package_name = tauri_config
        .product_name
        .clone()
        .unwrap_or_else(|| "nextbitdeskify".to_string());
    ...
    if window_config.new_window {
        ...
        window_builder = window_builder.on_new_window(move |target_url, features| {
            match open_requested_window(
                ...
                Err(error) => {
                    eprintln!("[NextBitDeskify] Failed to open requested window: {error}");
    ...
    // Add initialization scripts. Order matters: nextbitdeskifyConfig must land before
    // any script that reads it (e.g. fullscreen polyfill checks for an opt-out
    // flag), and toast must register `window.nextbitdeskifyToast` before Rust code
    // calls show_toast().
        ...
        window_builder = window_builder.on_download(move |_webview, event| match event {
            DownloadEvent::Requested { url, destination } => {
                match download_handle.path().download_dir() {
                    ...
                    Err(error) => {
                        eprintln!("[NextBitDeskify] Failed to resolve download dir: {error}");
                    }
                }
                true
            }
            DownloadEvent::Finished {
                ...
                if let Some(window) = download_handle.get_webview_window("nextbitdeskify") {
