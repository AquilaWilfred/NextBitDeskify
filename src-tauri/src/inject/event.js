...
function shouldBypassNextBitDeskifyLinkHandling(rawHref) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (internalUrlRegex) {
    try {
      ...
      console.error("[NextBitDeskify] Invalid internal_url_regex pattern:", e);
    }
  }

  if (!document.getElementById("nextbitdeskify-top-dom")) {
    const topDom = document.createElement("div");
    topDom.id = "nextbitdeskify-top-dom";
    document.body.appendChild(topDom);
  }

  const domEl = document.getElementById("nextbitdeskify-top-dom");
  ...
  if (window["nextbitdeskifyConfig"]?.disabled_web_shortcuts !== true) {
  ...
  const isInternalUrl = (url) => {
    ...
    if (internalUrlPattern) {
      try {
        ...
        console.error("[NextBitDeskify] Error testing internal_url_regex:", e);
  ...
  const detectAnchorElementClick = (e) => {
    ...
    if (anchorElement && anchorElement.href) {
      ...
      if (shouldBypassNextBitDeskifyLinkHandling(rawHref)) {
        return;
      }
      ...
      if (window.isAuthLink(absoluteUrl)) {
        ...
        if (window.nextbitdeskifyConfig?.new_window) {
          openAuthNavigation(
            originalWindowOpen,
            absoluteUrl,
            "_blank",
  ...
  window.open = function (url, name, specs) {
    ...
    if (shouldBypassNextBitDeskifyLinkHandling(url)) {
  ...
  const MENU_CONFIG = {
    id: "nextbitdeskify-context-menu",
...
// Bridge the Web Notification + Web Badging APIs to NextBitDeskify's Rust commands so
