...
use util::get_nextbitdeskify_config;
...
pub fn run_app() {
    ...
    let (nextbitdeskify_config, tauri_config) = get_nextbitdeskify_config();
    let tauri_app = tauri::Builder::default();

    let show_system_tray = nextbitdeskify_config.show_system_tray();
    let hide_on_close = nextbitdeskify_config.windows[0].hide_on_close;
    let activation_shortcut = nextbitdeskify_config.windows[0].activation_shortcut.clone();
    let init_fullscreen = nextbitdeskify_config.windows[0].fullscreen;
    let start_to_tray = nextbitdeskify_config.windows[0].start_to_tray && show_system_tray; // Only valid when tray is enabled
    let multi_instance = nextbitdeskify_config.multi_instance;
    let multi_window = nextbitdeskify_config.multi_window;
    let _enable_find = nextbitdeskify_config.windows[0].enable_find;
    ...
    if !multi_instance {
        app_builder = app_builder.plugin(tauri_plugin_single_instance::init(
            move |app, _args, _cwd| {
                ...
                } else if let Some(window) = app.get_webview_window("nextbitdeskify") {
    ...
    app_builder
        ...
        .setup(move |app| {
            ...
            let window = set_window(app.app_handle(), &nextbitdeskify_config, &tauri_config)?;
            set_system_tray(
                app.app_handle(),
                show_system_tray,
                &nextbitdeskify_config.system_tray_path,
                init_fullscreen,
                multi_window,
            )?;
        ...
        .on_window_event(move |_window, _event| {
            if let tauri::WindowEvent::CloseRequested { api, .. } = _event {
                if hide_on_close && _window.label() == "nextbitdeskify" {
        ...
        .unwrap_or_else(|error| {
            eprintln!("[NextBitDeskify] Fatal error while building Tauri application: {error}");
            std::process::exit(1);
        })
        .run(|_app, _event| {
            ...
            } = _event
            {
                if !has_visible_windows {
                    if let Some(window) = _app.get_webview_window("nextbitdeskify") {
                        let _ = window.show();
                        let _ = window.set_focus();
                    }
                }
