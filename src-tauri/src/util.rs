...
pub fn get_nextbitdeskify_config() -> (NextBitDeskifyConfig, Config) {
    ...
    (nextbitdeskify_config, tauri_config)
}
...
pub fn show_toast(window: &WebviewWindow, message: &str) {
    let script = format!(r#"nextbitdeskifyToast("{message}");"#);
    if let Err(error) = window.eval(&script) {
        eprintln!("[NextBitDeskify] Failed to show toast: {error}");
    }
}
