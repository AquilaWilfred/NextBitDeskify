...
/**
 * Unified Test Runner for NextBitDeskify CLI
 *
...
class NextBitDeskifyTestRunner {
  constructor() {
    this.results = [];
    this.tempFiles = [];
    this.tempDirs = [];
  ...
  async runAll(options = {}) {
    const {
      ...
      integration = true,
      builder = true,
      nextbitdeskifyCliTests = false,
      e2e = false,
      quick = false,
      realBuild = false, // Add option for real build test
    } = options;

    console.log("NextBitDeskify CLI Test Suite");
    console.log("======================\n");
  ...
  async runNextBitDeskifyCliTests() {
    // Package installation test
    await this.runTest(
      "nextbitdeskify-cli Package Installation",
      async () => {
        try {
          execSync("pnpm install nextbitdeskify-cli@latest", {
            encoding: "utf8",
            timeout: 60000,
            cwd: "/tmp",
          });

          const nextbitdeskifyCliPath = "/tmp/node_modules/.bin/nextbitdeskify";
          return fs.existsSync(nextbitdeskifyCliPath);
        } catch (error) {
    ...
    // Version command test
    await this.runTest("nextbitdeskify-cli Version Command", async () => {
      try {
        const version = execSync("npx nextbitdeskify --version", {
          encoding: "utf8",
          timeout: 10000,
        });
            ...
            if (fs.existsSync(nextbitdeskifyDir)) {
              ...
              if (fs.existsSync(configFile) && fs.existsSync(nextbitdeskifyConfigFile)) {
                try {
                  ...
                  const nextbitdeskifyConfig = JSON.parse(
                    fs.readFileSync(nextbitdeskifyConfigFile, "utf8"),
                  );
  ...
  async runLocalFileTest() {
    await this.runTest("Local File Build Handling", async () => {
      ...
      fs.writeFileSync(
        testFile,
        "<html><body><h1>Hello NextBitDeskify</h1></body></html>",
      );
      this.trackTempFile(testFile);
        ...
        return new Promise((resolve, reject) => {
          ...
          const outputFiles = {
            ...
            linux: {
              app: path.join(
                config.PROJECT_ROOT,
                `src-tauri/target/release/nextbitdeskify`,
              ),
              installer: path.join(
                config.PROJECT_ROOT,
      ...
      try {
        ...
        const buildFiles = items.filter((item) => {
          ...
          return (
            ...
            (platform === "linux" && item === "nextbitdeskify")
          ); // Linux binary
        });
        ...
        buildFiles.forEach((file) => {
          ...
          else if (file === "nextbitdeskify") fileType = "Linux Binary";
...
const options = {
  ...
  nextbitdeskifyCliTests: args.includes("--nextbitdeskify-cli"),
...
if (args.includes("--help") || args.includes("-h")) {
  console.log(`
[Run] NextBitDeskify CLI Test Suite
...
Optional Components:
  ...
  --nextbitdeskify-cli     Add nextbitdeskify-cli GitHub Actions tests
...
// Run tests
const runner = new NextBitDeskifyTestRunner();
runner
  .runAll(options)
  .then(async (success) => {
