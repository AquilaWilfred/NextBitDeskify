...
function loadFindScript({
  ...
  const context = {
    ...
    window: {
      nextbitdeskifyConfig: { enable_find: enabled },
    },
...
describe("Find injection", () => {
  it("does not register shortcuts when enable_find is false", () => {
    ...
    expect(context.window.nextbitdeskifyFind.getState().enabled).toBe(false);
    expect(
      context.window.nextbitdeskifyFind.getFindShortcutAction(
  ...
  it("handles Cmd/Ctrl+F and Cmd/Ctrl+G shortcuts when enabled", () => {
    ...
    const calls = [];
    context.window.nextbitdeskifyFind.open = () => calls.push("open");
    context.window.nextbitdeskifyFind.next = () => calls.push("next");
    context.window.nextbitdeskifyFind.previous = () => calls.push("previous");
  ...
  it("counts text matches and skips input and script content", () => {
    ...
    const result = context.window.nextbitdeskifyFind.search("alpha");
  ...
  it("clears matches on Escape", () => {
    ...
    context.window.nextbitdeskifyFind.search("alpha");
    expect(context.window.nextbitdeskifyFind.getState().matchCount).toBe(2);

    context.window.nextbitdeskifyFind.close();
    expect(context.window.nextbitdeskifyFind.getState().matchCount).toBe(0);
  });
});
